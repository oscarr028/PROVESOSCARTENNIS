...
st.set_page_config(page_title="PROVES OSCAR", layout="wide")
...
with tab2:
    ...
    def explain_row_contributions(
        ...
        """
        Retorna una explicació local (una fila):
          - contribucions per feature en log-odds (model base)
          - top pos/neg
          - prob base (raw) i prob calibrada (si iso)
        Compatible amb:
          - LogisticRegression (predict_proba) + StandardScaler
          - LightGBM Booster (predict) amb pred_contrib=True
        """
        if ds_row is None or len(ds_row) != 1:
            return {"ok": False, "error": "ds_row ha de tenir exactament 1 fila."}
    
        # 1) prepara X en l’ordre exacte del model
        Xdf = ds_row.reindex(columns=model_cols, fill_value=0.0).copy()
        Xdf = Xdf.apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(0.0)
    
        # ---- Case A: LightGBM Booster ----
        # (No té predict_proba típicament; i suporta pred_contrib)
        if hasattr(model, "predict") and not hasattr(model, "predict_proba"):
            try:
                contrib = model.predict(Xdf.values, pred_contrib=True)
                # shape: (1, n_features+1) -> últim és bias
                ...
                cvals = contrib[0, :-1]
                logit = bias + float(np.sum(cvals))
                p_raw = _sigmoid(logit)
                p_cal = float(iso.transform([p_raw])[0]) if iso is not None else p_raw
    
                dfc = pd.DataFrame({
                    "feature": model_cols,
                    "value": Xdf.iloc[0].values,
                    "contrib_logodds": cvals,
                })
            except Exception as e:
                return {"ok": False, "error": f"LightGBM pred_contrib failed: {type(e).__name__}: {e}"}
    
        # ---- Case B: LogisticRegression (o similar) ----
        else:
            if not hasattr(model, "coef_"):
                return {"ok": False, "error": "Model no suportat per explain (falten coef_ o pred_contrib)."}
    
            # estandarditza igual que training
            X = Xdf.values
            if scaler is not None and hasattr(scaler, "transform"):
                Xs = scaler.transform(X)
            else:
                # si no hi ha scaler, tractem X com ja “en escala”
                Xs = X
        ...
        # ranking
        dfc["abs_contrib"] = dfc["contrib_logodds"].abs()
        dfc = dfc.sort_values("abs_contrib", ascending=False)
    
        top = dfc.head(top_k).copy()
        top_pos = dfc[dfc["contrib_logodds"] > 0].head(top_k).copy()
        top_neg = dfc[dfc["contrib_logodds"] < 0].head(top_k).copy()
    ...
    if "last_manual_tip" in st.session_state and isinstance(st.session_state["last_manual_tip"], pd.DataFrame):
        ...
        # IMPORTANT: necessites la fila de features (dataset) que has usat per predir.
        # Si ja tens ds_fx (1 fila) abans de fer predict, reutilitza-la aquí.
